...
echo "== extension"; MDR_CHROME="$CFT" node test/ext-test.mjs http://127.0.0.1:8477/sample.md "$OUT/ext" | grep -E '"(mounted|tables|filesHidden|editRendered|pinchOk|buttonsOk|widthOk)"|EXC'; E=${PIPESTATUS[0]}
kill $SRV 2>/dev/null
echo "== app"; node test/app-test.mjs "$OUT/app" | grep -E '"(mounted|defaultTheme|stillMounted|outlineToggleOk|tabsOk|pathLinksOk|imageOk|pinchAnchorOk|windows|secondFileUntouched|filesPanelVisible|savedToDisk|dirtyAfterSave|reloadedFromDisk)"|EXC'; A=${PIPESTATUS[0]}
