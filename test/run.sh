...
echo "== json"; MDR_CHROME="$CFT" node test/json-test.mjs http://127.0.0.1:8477 "$OUT/json" | grep -E '"(mounted|kind|brokenMounted|multiOk|jsonOk)"|EXC'; J=${PIPESTATUS[0]}
kill $SRV 2>/dev/null
...
echo "ext rc=$E  json rc=$J  app rc=$A  session rc=$S  screenshots: $OUT"; [ $E -eq 0 ] && [ $J -eq 0 ] && [ $A -eq 0 ] && [ $S -eq 0 ]
